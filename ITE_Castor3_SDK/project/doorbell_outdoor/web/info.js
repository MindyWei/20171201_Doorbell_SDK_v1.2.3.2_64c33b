...
function updateIndoorSetting() {
    if(lockUI)
        return;

    var $engineer_password = document.getElementById("engineer_password").value;
    var $door_password     = document.getElementById("door_password").value;
    var $brightness        = document.getElementById("brightness").value;
    var $screensaver_time  = document.getElementById("screensaver_time").value;
    var $ring_lev          = document.getElementById("ring_lev").value;
    var $keylevel          = document.getElementById("keylevel").value;
    var $play_lev          = document.getElementById("play_lev").value;
    var $rec_lev           = document.getElementById("rec_lev").value;
    ...
    var $ring_type = "ring1.wav";
    switch(document.getElementById("ring_type").selectedIndex) {
        case 0:
            $ring_type = "ring1.wav";
            break;
        case 1:
    ...
    if (confirm("Do you want to update setting?")==true) {
        ...
        var nocache = "&nocache=" + Math.random() * 1000000;
        var hostStr = "http://" + location.host + "/dev/info.cgi?action=setting&engineer_password=" + $engineer_password + "&door_password=" + $door_password + "&brightness=" + $brightness + "&screensaver_time=" + $screensaver_time + "&screensaver_type=" + $screensaver_type + "&lang=" + $lang + "&ring_lev=" + $ring_lev + "&ring_type=" + $ring_type + "&keylevel=" + $keylevel + "&keysound_type=" + $keysound_type + "&play_lev=" + $play_lev + "&rec_lev=" + $rec_lev + "&area=" + $area + "&building=" + $building + "&unit=" + $unit + "&floor=" + $floor + "&room=" + $room + "&ext=" + $ext + nocache;
...
function updateOutdoorSetting() {
    ...
    if (confirm("Do you want to update setting?")==true) {
        ...
        var nocache = "&nocache=" + Math.random() * 1000000;
        var hostStr = "http://" + location.host + "/dev/info.cgi?action=setting&area=" + $area + "&building=" + $building + "&unit=" + $unit + "&floor=" + $floor + "&room=" + $room + "&ext=" + $ext + nocache;
...
function updateEntranceSetting() {
    if(lockUI)
        return;

    var $local_pos  = document.getElementById("local_pos").value;
    var $local_ext  = document.getElementById("local_ext").value;
    var $local_type = document.getElementById("local_type").value;
    var $rec_lev    = document.getElementById("rec_lev").value;
    var $ring_lev   = document.getElementById("ring_lev").value;
    var $play_lev   = document.getElementById("play_lev").value;
    ...
    if (confirm("Do you want to update setting?")==true) {
        ...
        var hostStr = "http://" + location.host + "/dev/info.cgi?action=setting&local_pos=" + $local_pos + "&local_ext=" + $local_ext + "&local_type=" + $local_type + "&rec_lev=" + $rec_lev  + "&ring_lev=" + $ring_lev + "&play_lev=" + $play_lev + nocache;

        //alert("hostStr: " + hostStr);
        ajaxSendRequest(hostStr, true, onRcvDataUpdateSetting);
