    ...
    char            *filename)
{
...
#if 0
    {
        ...
        fwrite(outStreamInfo.jstream.mem[0].pAddr,
            1, outStreamInfo.jstream.mem[0].length, fout);
        fclose(fout);

        snprintf(out_path, 256, "A:/dump_u_%ux%u.raw", real_width, outStreamInfo.jstream.mem[1].pitch);
        if( !(fout = fopen("A:/dump_u.raw", "w")) )
        {
            printf("open dump_u fail !\n");
            while(1);
        }
        fwrite(outStreamInfo.jstream.mem[1].pAddr,
            1, outStreamInfo.jstream.mem[1].length, fout);
        fclose(fout);

        snprintf(out_path, 256, "A:/dump_v_%ux%u.raw", real_width, outStreamInfo.jstream.mem[2].pitch);
        if( !(fout = fopen("A:/dump_v.raw", "w")) )
        {
            printf("open dump_v fail !\n");
            while(1);
        }
        fwrite(outStreamInfo.jstream.mem[2].pAddr,
            1, outStreamInfo.jstream.mem[2].length, fout);
        fclose(fout);
    }
#endif
    {
