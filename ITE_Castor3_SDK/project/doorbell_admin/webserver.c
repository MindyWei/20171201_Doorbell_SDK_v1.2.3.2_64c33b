          ...
          const char *upload_data, size_t *upload_data_size, void **ptr)
{
    ...
    if (NULL == request)
    {
        ...
        *ptr = request;
        ...
        if (0 == strcmp (method, MHD_HTTP_METHOD_POST))
        {
            ...
            else if (strcmp(url, "/doorbell/upload_event") == 0)
                request->key = "upload_event";
    ...
    if (strcmp(url, "/dev/info.cgi") == 0)
    {
        if (0 == strcmp (method, MHD_HTTP_METHOD_GET))
        {
            ...
            *ptr = NULL;                  /* reset when done */
            ...
            else if (strcmp(val, "upgrade_userlist") == 0)
            {
                const char* val;
                ...
                UpgradeSetUrl((char*)val);
                SceneQuit(QUIT_UPGRADE_USERLIST);
    ...
    else if (strcmp(url, "/doorbell/upload_event") == 0)
    {
        if (0 == strcmp (method, MHD_HTTP_METHOD_POST))
        {
            ...
            puts("recv event finish.");

            GuardParseLog(request->filebuf, request->filesize);
            GuardSaveLog();
