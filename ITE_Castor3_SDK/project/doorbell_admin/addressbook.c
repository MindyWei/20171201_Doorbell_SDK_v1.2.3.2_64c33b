...
int AddressBookGetMiscInfoArray(DeviceInfo infoArray[], int count)
{
    ...
    for (i = 0; i < abDeviceCount; i++)
    {
        ...
        if (dev->type == DEVICE_OUTDOOR || dev->type == DEVICE_ENTRANCE || dev->type == DEVICE_BUILDING || dev->type == DEVICE_WALL || dev->type == DEVICE_INTERCOM)
        {
            DeviceInfo* info = &infoArray[j];
            GetDeviceInfo(info, dev);
            j++;
...
bool AddressBookIsIntercomExist(char* area, char* building, char* unit, char* floor, char* room, char* ext)
{
    ...
    for (i = 0; i < abDeviceCount; i++)
    {
        ...
        if (dev->type == DEVICE_INTERCOM && dev->room[0] == area_i && dev->room[1] == building_i && dev->room[2] == unit_i && dev->room[3] == floor_i && dev->room[4] == room_i && dev->room[5] == ext_i)
        {
            return true;
        }
    }
