...
static int UpgradeResetTempDrive(void)
{
    ...
    if (UpgradeGetSDPath())
    {
        ioctl(ITP_DEVICE_FAT, ITP_IOCTL_FORMAT, (void*)3);
    }
...
{
    const char* docStr = "<?xml version=\"1.0\" encoding=\"utf-8\"?><DeviceInfo><firmware/><addressbook/><userlist/><do_not_disturb/></DeviceInfo>";
    ...
    xpathObj = xmlXPathEvalExpression(BAD_CAST "/DeviceInfo/userlist", xpathCtx);
    if (!xmlXPathNodeSetIsEmpty(xpathObj->nodesetval))
    {
        char buf[32];
        xmlNodePtr node = xpathObj->nodesetval->nodeTab[0];
        int val = UserListGetVersion();
...
static void* UpgradeProcessTask(void* arg)
{
    ...
    else if (code == QUIT_UPGRADE_USERLIST)
    {
        UserListReload();
    }
end:
...
int UpgradeProcess(int code)
{
    ...
    else if (code == QUIT_UPGRADE_ADDRESSBOOK || code == QUIT_UPGRADE_USERLIST)
    {
        pthread_t task;
        pthread_attr_t attr;
