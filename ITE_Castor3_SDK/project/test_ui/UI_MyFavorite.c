...
static ITUCoverFlow *favoriteCoverFlow = 0;
...
void RefreshFavoriteList()
{
	...
	if (i == 0) {
		ituWidgetSetVisible(favoriteCoverFlow, false);
	}
	else {
		ituWidgetSetVisible(favoriteCoverFlow, true);
	}
}
...
bool MainOnEnter(ITUWidget* widget, char* param)
{
	...
	if (!mainCoverflowIcon[0])
	{
		...
		favoriteCoverFlow = ituSceneFindWidget(&scene, "favoriteCoverFlow");
		assert(favoriteCoverFlow);
