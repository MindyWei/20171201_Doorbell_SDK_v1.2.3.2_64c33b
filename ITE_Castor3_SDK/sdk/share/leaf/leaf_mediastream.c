...
LeafCall* leaf_init(void) {
...
#if defined(CFG_BROADCAST_SERVER_ENABLE)
    call->capt_sndcard = get_sndcard(MS_SND_CARD_CAP_CAPTURE);
#elif defined(CFG_BROADCAST_CLIENT_ENABLE)
    ...
    call->cfgini = iniparser_load(CFG_PUBLIC_DRIVE ":/Esound.ini");
#else
...
void leaf_start_audio_stream(LeafCall *call, const char *addr, int port) {
	bool_t use_ec = call->audiostream->use_ec;
