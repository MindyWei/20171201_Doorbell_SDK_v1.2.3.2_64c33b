...
void stepwheel_set_wheel_font_size(ITUStepWheel* wheel, ITUText* text, int size)
{
...
void stepwheel_wheel_font_size_cal(ITUStepWheel* wheel)
{
	...
	if (wheel->cycle_tor == 0)
	{
		...
		for (i = 0; i < max_stage_size; i++)
		{
			...
			if (prev_index >= 0)
				textpp[i] = (ITUText*)itcTreeGetChildAt(wheel, wheel->focusIndex - (i + 1));
			else
				textpp[i] = NULL;

			next_index = wheel->focusIndex + (i + 1);
			if (next_index <= (count - 1))
				textnn[i] = (ITUText*)itcTreeGetChildAt(wheel, wheel->focusIndex + (i + 1));
			else
				textnn[i] = NULL;
		}
	}
	...
	if (wheel->focus_dev && (wheel->frame == wheel->totalframe))
	{
		...
		if (wheel->focus_dev > 0)
			text = (ITUText*)itcTreeGetChildAt(wheel, wheel->focusIndex + 1);
		else
			text = (ITUText*)itcTreeGetChildAt(wheel, wheel->focusIndex - 1);
