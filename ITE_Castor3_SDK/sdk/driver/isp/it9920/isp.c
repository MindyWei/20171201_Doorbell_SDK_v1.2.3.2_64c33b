...
#else
MMP_FLOAT ScaleRatio[WEIGHT_NUM] = {(800.0f / 640), (1088.0f / 1080), (1920.0f / 1920), (1280.0f / 1920), (576.0f / 1080), (480.0f / 1080), (720.0f / 1920), (640.0f / 1920), (288.0f / 1080), (352.0f / 1920) };
#endif
...
MMP_UINT8 WeightMatInt2[WEIGHT_NUM][ISP_SCALE_TAP_SIZE][ISP_SCALE_TAP];
    ...
    ISP_DEVICE ptDev)
{
    ...
    if (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == true && pISPctxt->PreScaleSel == MMP_ISP_PRESCALE_PREVIEW)
    ...
    ISP_DEVICE ptDev)
{
    ...
    if (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == true && pISPctxt->PreScaleSel == MMP_ISP_PRESCALE_NORMAL)
    ...
    MMP_ISP_CORE_INFO *coreInfo)
{
    ...
    // VP1 API
    pISPctxt->ispCore = coreInfo->core;
    ...
    // VP1 API
    if (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == true)
    {
        pISPctxt->PVOutInfo.OutFormat   = RGBPacket;
        pISPctxt->PVOutInfo.RGBFormat   = Dither565;
        pISPctxt->PVOutInfo.DitherMode  = false;
    ...
    else if (pISPctxt->ispCore == MMP_ISP_CORE_1)
    {
        for (index = 0; index < WEIGHT_NUM; index++)
        {
            ...
            for (j = 0; j < ISP_SCALE_TAP_SIZE; j++)
                for (i = 0; i < ISP_SCALE_TAP; i++)
                    WeightMatInt2[index][j][i] = (MMP_UINT8)ISP_FloatToFix(WeightMat[j][i], 1, 6);
        }
    }
    ...
    ISP_DEVICE ptDev)
{
        ...
        (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_InputBuf))
    {
        ...
        if (pISPctxt->ispCore == MMP_ISP_CORE_0 || (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == false))
            pISPctxt->PreScaleFun.DstWidth = _MIN(pISPctxt->InInfo.PanelWidth, ISP_SCALE_MAX_LINE_BUFF_LEN);
        else if (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == true && pISPctxt->PreScaleSel == MMP_ISP_PRESCALE_PREVIEW)
            pISPctxt->PreScaleFun.DstWidth = _MIN(pISPctxt->InInfo.PanelWidth, ISP_SCALE_MAX_LINE_BUFF_LEN_PREVIEW);
        else if (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == true && pISPctxt->PreScaleSel == MMP_ISP_PRESCALE_NORMAL)
    ...
    ISP_DEVICE ptDev)
{
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_EngineMode || pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_OutParameter)
        ISP_SetEngineModeOutParameter_Reg(&pISPctxt->EngineMode, &pISPctxt->OutInfo, pISPctxt->ispCore);
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_YUVtoRGBMatrix)
        ISP_SetYUVtoRGBMatrix_Reg(pISPctxt->YUV2RGBFun, pISPctxt->ispCore);

    //
    //Color Correction Parameter
    //
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_CCMatrix)
        ISP_SetCCMatrix_Reg(&pISPctxt->CCFun, pISPctxt->ispCore);
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PreScaleParam)
    {
        ...
        ISP_SetPreScaleParam_Reg(&pISPctxt->PreScaleFun, pISPctxt->ispCore);
    }
        ...
        //{
            if (pISPctxt->ispCore == MMP_ISP_CORE_0)
            {
                for (index = 0; index < WEIGHT_NUM; index++)
                {
                    ...
                    {
                        ISP_SetIntPreScaleMatrix_Reg(WeightMatInt[index], pISPctxt->ispCore);
                        break;
                    }
            ...
            else if (pISPctxt->ispCore == MMP_ISP_CORE_1)
            {
                for (index = 0; index < WEIGHT_NUM; index++)
                {
                    if (pISPctxt->PreScaleFun.HCI >= ScaleRatio[index])
                    {
                        ISP_SetIntPreScaleMatrix_Reg(WeightMatInt2[index], pISPctxt->ispCore);
                        break;
                    }
                    else if (index == WEIGHT_NUM - 1)
                        ISP_SetIntPreScaleMatrix_Reg(WeightMatInt2[index], pISPctxt->ispCore);
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_ScaleParam)
    {
        ...
        ISP_SetScaleParam_Reg(&pISPctxt->ScaleFun, pISPctxt->ispCore);
    }
        ...
        //{
            if (pISPctxt->ispCore == MMP_ISP_CORE_0)
            {
                for (index = 0; index < WEIGHT_NUM; index++)
                {
                    ...
                    {
                        ISP_SetIntScaleMatrixH_Reg(WeightMatInt[index], pISPctxt->ispCore);
                        break;
                    }
                    else if (index == WEIGHT_NUM - 1)
                        ISP_SetIntScaleMatrixH_Reg(WeightMatInt[index], pISPctxt->ispCore);
            ...
            else if (pISPctxt->ispCore == MMP_ISP_CORE_1)
            {
                for (index = 0; index < WEIGHT_NUM; index++)
                {
                    if (pISPctxt->ScaleFun.HCI >= ScaleRatio[index])
                    {
                        ISP_SetIntScaleMatrixH_Reg(WeightMatInt2[index], pISPctxt->ispCore);
                        break;
                    }
                    else if (index == WEIGHT_NUM - 1)
                        ISP_SetIntScaleMatrixH_Reg(WeightMatInt2[index], pISPctxt->ispCore);
        ...
        //{
            if (pISPctxt->ispCore == MMP_ISP_CORE_0)
            {
                for (index = 0; index < WEIGHT_NUM; index++)
                {
                    ...
                    {
                        ISP_SetIntScaleMatrixV_Reg(WeightMatInt[index], pISPctxt->ispCore);
                        break;
                    }
                    else if (index == WEIGHT_NUM - 1)
                        ISP_SetIntScaleMatrixV_Reg(WeightMatInt[index], pISPctxt->ispCore);
            ...
            else if (pISPctxt->ispCore == MMP_ISP_CORE_1)
            {
                for (index = 0; index < WEIGHT_NUM; index++)
                {
                    if (pISPctxt->ScaleFun.VCI >= ScaleRatio[index])
                    {
                        ISP_SetIntScaleMatrixV_Reg(WeightMatInt2[index], pISPctxt->ispCore);
                        break;
                    }
                    else if (index == WEIGHT_NUM - 1)
                        ISP_SetIntScaleMatrixV_Reg(WeightMatInt2[index], pISPctxt->ispCore);
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_FrmMatrix)
        ISP_SetFrmMatrix_Reg(&pISPctxt->FrmMatrix, pISPctxt->ispCore);

    //
    // Frame Function 0
    //
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_FrameFun0)
        ISP_SetFrameFun0_Reg(&pISPctxt->FrameFun0, pISPctxt->ispCore);

    //
    // Frame Function 1
    //
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_FrameFun1)
        ISP_SetFrameFun1_Reg(&pISPctxt->FrameFun1, pISPctxt->ispCore);

    //
    // YUV to RGB Parameter
    //
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_RGBtoYUVMatrix)
        ISP_SetRGBtoYUVMatrix_Reg(pISPctxt->RGB2YUVFun, pISPctxt->ispCore);
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_Clip0Fun)
    {
        ...
        ISP_SetClip0Fun_Reg(&pISPctxt->ClipFun0, pISPctxt->ispCore);
    }
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_OutAddress || pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_OutBufInfo)
        ISP_SetOutAddressBufInfo_Reg(&pISPctxt->OutInfo, pISPctxt->ispCore);
    ...
    if (pISPctxt->ispCore == MMP_ISP_CORE_1)
    {
        ...
        if (pISPctxt->EnPreview == true)
        {
            if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVScaleParam)
            {
                _ISP_CalPVScaleHCI_VCI(pISPctxt);
                ISP_SetPVScaleParam_Reg(&pISPctxt->PVScaleFun, pISPctxt->ispCore);
            }
            ...
            if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVScaleMatrixH)
            {
                ...
                //{
                    for (index = 0; index < WEIGHT_NUM; index++)
                    {
                        ...
                        {
                            ISP_SetIntPVScaleMatrixH_Reg(WeightMatInt2[index], pISPctxt->ispCore);
                            break;
                        }
                        else if (index == WEIGHT_NUM - 1)
                            ISP_SetIntPVScaleMatrixH_Reg(WeightMatInt2[index], pISPctxt->ispCore);
            ...
            if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVScaleMatrixV)
            {
                ...
                //{
                    for (index = 0; index < WEIGHT_NUM; index++)
                    {
                        ...
                        {
                            ISP_SetIntPVScaleMatrixV_Reg(WeightMatInt2[index], pISPctxt->ispCore);
                            break;
                        }
                        else if (index == WEIGHT_NUM - 1)
                            ISP_SetIntPVScaleMatrixV_Reg(WeightMatInt2[index], pISPctxt->ispCore);
            ...
            if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVOutParameter)
                ISP_SetPVOutParameter_Reg(&pISPctxt->PVOutInfo, pISPctxt->ispCore);

            if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVOutAddress || pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVOutBufInfo)
                ISP_SetPVOutAddressBufInfo_Reg(&pISPctxt->PVOutInfo, pISPctxt->ispCore);
        }
    }
}
