    ...
    ISP_DEVICE *ptDev2)
{
    ...
    if ((*ptDev2) == MMP_NULL)
        *ptDev2 = isp_Malloc(sizeof(ISP_CONTEXT));
    ...
    ISP_DEVICE ptDev2)
{
    ...
    ISP_CONTEXT *pISPctxt2 = (ISP_CONTEXT *)ptDev2;

    GOTO_END_IF_NOT_INITIALIZE(result, pISPctxt);
    GOTO_END_IF_NOT_INITIALIZE(result, pISPctxt2);
    ...
    isp_Memset((void *)pISPctxt2, 0, sizeof(ISP_CONTEXT));
    ...
    ISP_DEVICE *ptDev2)
{
    ...
    if (ptDev == MMP_NULL || pISPctxt == MMP_NULL || ptDev2 == MMP_NULL || pISPctxt2 == MMP_NULL)
    {
        return result;
    }
    ...
    result = ISP_WaitEngineIdle(pISPctxt2);
    if (result)
    {
        isp_msg_ex(ISP_MSG_TYPE_ERR, " err 0x%x !\n", result);
        goto end;
    ...
    MMP_ISP_OUTPUT_INFO *outInfo)
{
    ...
    case MMP_ISP_OUT_YUV422:
        ...
        pISPctxt->PVOutInfo.PitchUV   = (MMP_UINT16)outInfo->pitchUv;
        pISPctxt->PVOutInfo.Width     = (MMP_UINT16)outInfo->width;
        pISPctxt->PVOutInfo.Height    = (MMP_UINT16)outInfo->height;

        pISPctxt->PVOutInfo.Addr0     = (MMP_UINT8 *)outInfo->addrY;
        pISPctxt->PVOutInfo.Addr1     = (MMP_UINT8 *)outInfo->addrU;
        pISPctxt->PVOutInfo.Addr2     = (MMP_UINT8 *)outInfo->addrV;
        break;
    ...
    MMP_ISP_OUTFORMAT format)
{
    ...
    case MMP_ISP_OUT_YUV422:
        pISPctxt->PVOutInfo.OutFormat     = YUVPlane;
        pISPctxt->PVOutInfo.PlaneFormat   = YUV422;
        break;

    case MMP_ISP_OUT_YUV420:
        pISPctxt->PVOutInfo.OutFormat     = YUVPlane;
        pISPctxt->PVOutInfo.PlaneFormat   = YUV420;
        break;

    case MMP_ISP_OUT_YUV444:
        pISPctxt->PVOutInfo.OutFormat     = YUVPlane;
        pISPctxt->PVOutInfo.PlaneFormat   = YUV444;
        break;

    case MMP_ISP_OUT_YUV422R:
        pISPctxt->PVOutInfo.OutFormat     = YUVPlane;
        pISPctxt->PVOutInfo.PlaneFormat   = YUV422R;
        break;
    ...
    const MMP_ISP_SHARE *data)
{
    ...
    ISP_DriverFire_Reg(&pISPctxt->OutInfo, pISPctxt->ispCore);
    ...
    const MMP_ISP_SHARE *data)
{
    ...
    if (pISPctxt->ispMode == MMP_ISP_MODE_PLAY_VIDEO && pISPctxt->OutInfo.EnableLcdOnFly == MMP_FALSE)
    {
        if (pISPctxt->OutInfo.EnableQueueFire)
        {
            if (pISPctxt->WorkFlags & ISP_FLAGS_QUEUE_FIRE_INIT)
            {
                ...
                mmpIspSetWriteBufferNum(ptDev, !(status & 0x00001000));
                pISPctxt->WorkFlags &= ~ISP_FLAGS_QUEUE_FIRE_INIT;
            }
            else
            {
    ...
    if (pISPctxt->ispMode == MMP_ISP_MODE_PLAY_VIDEO)
    {
        if (pISPctxt->OutInfo.EnableLcdOnFly)
        {
            ...
            {
                ISP_UpdateFire_Reg(&pISPctxt->OutInfo, pISPctxt->ispCore);
            }
        }
        else
        {
    ...
    ISP_DEVICE ptDev)
{
    ...
    //ISP_LogReg(pISPctxt->ispCore);
    ISP_WirteRawSliceFire_Reg(&pISPctxt->OutInfo, pISPctxt->ispCore);
